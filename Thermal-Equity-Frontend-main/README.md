...
The three GCC boundary GeoPackages are present and validated. The saved `.qgz` remains readable but contains zero saved map layers.
...
The frontend uses React with Vite, HMR, and Oxlint rules. The React Compiler is not enabled because of its impact on build performance. For production applications, consider the TypeScript template with type-aware lint rules.

## AI/ML Module - Thermal Equity Risk Prediction

### Objective

The AI/ML module predicts the thermal risk category of a geographical area based on environmental, demographic, and accessibility indicators.

### Input features

The model uses latitude, longitude, LST, NDVI, built-up percentage, green cover percentage, population density, vulnerable population percentage, water access percentage, and cooling access percentage.

### Machine learning models

Random Forest and XGBoost were evaluated. Based on the weighted F1 score, Random Forest was selected as the final model.
...
SHAP was used to analyze feature contributions. The visualization is stored at `ml-service/data/processed/shap_summary.png`.

### API integration

The trained model is integrated with the FastAPI service in `ml-service/api/main.py`. The `POST /predict` endpoint accepts the ten environmental and demographic features and returns a risk category and prediction confidence.
