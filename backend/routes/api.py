    ...
    for idx, loc in enumerate(locations):
        try:
            ...
            if temp is not None and hum is not None:
                ...
                # Flag critical alert if high/extreme
...
@router.get(
    "/dashboard/summary",
    tags=["Dashboard"],
    description="Aggregated real-time metrics, latest thermal readings, and highest risk zones.",
)
def dashboard_summary(
    ...
    total_locations = database.scalar(select(func.count(Location.id))) or 0
    active_alerts = database.scalar(select(func.count(Alert.id)).where(Alert.status == "active")) or 0
    recent_measurements = database.scalar(select(func.count(ThermalData.id))) or 0
