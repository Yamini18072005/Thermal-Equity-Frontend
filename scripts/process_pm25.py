"""Map real Open-Meteo/CAMS PM2.5 source points to the common analysis grid."""
...
def main() -> None:
    ...
    if not args.input.is_file():
        raise SystemExit(f"WAITING_FOR_REAL_DATA: PM2.5 JSON not found: {args.input}")
    ...
    for item in document:
        ...
        values = item.get("hourly", {}).get("pm2_5", [])
        if args.hour in times:
            value = values[times.index(args.hour)]
            if value is not None:
                points.append((float(item["longitude"]), float(item["latitude"]), float(value)))
    if len(points) < 4:
        raise SystemExit("PM2.5 input has too few valid real source points.")
    with rasterio.open(args.reference) as reference:
        ...
        profile.update(driver="GTiff", dtype="float32", count=1, nodata=-9999, compress="deflate", units="micrograms per cubic metre", source_period=args.hour, method="nearest real CAMS/Open-Meteo source point; model spatial resolution is coarser than 30 m")
